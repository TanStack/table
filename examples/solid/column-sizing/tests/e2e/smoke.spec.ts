...
test('keeps all three table renderings the same width', async ({ page }) => {
  ...
  // The two div renderings size straight from the column size, so they land on
  // it exactly. The native table stretches to fill, so it is compared against
  // its siblings rather than against the declared number.
