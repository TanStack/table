...
interface TradingCellContext { row: { original: MarketQuote } }
...
export function createTradingColumns<TFeatures extends TableFeatures>(controller: TradingBenchmarkController): Array<ColumnDef<TFeatures, MarketQuote>> {
  const columns: Array<TradingColumnDefinition> = [
    { id: 'instrument', header: 'Instrument', columns: [
      { id: 'market', header: 'Market', size: 72, accessorFn: (row) => row.venue, cell: ({ row }) => recordCellRender('Market', row.original.venue) },
      { id: 'name', header: 'Name', size: 180, accessorFn: (row) => row.company, cell: ({ row }) => recordCellRender('Name', row.original.company) },
      { id: 'symbol', header: 'Symbol', size: 92, accessorFn: (row) => row.symbol, filterFn: 'includesString', cell: ({ row }) => recordCellRender('Symbol', row.original.symbol) },
    ] },
    { id: 'priceAndChange', header: 'Price & Change', columns: [
      { id: 'price', header: 'Price', size: 96, accessorFn: (row) => row.price, sortFn: 'basic', cell: ({ row }) => recordCellRender('Last', html`<quote-price-cell .price=${row.original.price} .move=${getDayChange(row.original)} .select=${() => controller.actions.selectSymbol(row.original.symbol)}></quote-price-cell>`) },
      { id: 'change', header: 'Chg', size: 94, accessorFn: getDayChange, cell: ({ row }) => recordCellRender('Change', renderMove(controller.renderAtoms.rendererMode.get(), getDayChange(row.original))) },
      { id: 'changePercent', header: 'Chg%', size: 90, accessorFn: getDayChangePercent, cell: ({ row }) => recordCellRender('ChangePercent', html`<quote-percent-change .value=${getDayChangePercent(row.original)}></quote-percent-change>`) },
    ] },
    ...
    { id: 'chart', header: 'Chart', columns: [
      { id: 'history', header: 'Intraday', size: 150, enableSorting: false, cell: ({ row }) => recordCellRender('Intraday', html`<quote-sparkline .values=${row.original.history}></quote-sparkline>`) },
    ] },
  ]
...
function renderMove(mode: RendererMode, move: number) {
  if (mode === 'stable') return html`<quote-stable-move .move=${move}></quote-stable-move>`
  return move >= 0 ? html`<quote-up-move .move=${move}></quote-up-move>` : html`<quote-down-move .move=${move}></quote-down-move>`
}
...
export function readMeasuredRows<Row>(readRows: () => Array<Row>): Array<Row> {
  const start = performance.now(); const rows = readRows(); const end = performance.now(); const duration = end - start
  rowModelDiagnostics.calls++; rowModelDiagnostics.hasMeasurement = true; rowModelDiagnostics.totalDurationMs += duration; rowModelDiagnostics.maxDurationMs = Math.max(rowModelDiagnostics.maxDurationMs, duration); rowModelDiagnostics.lastRowCount = rows.length
  try { performance.measure('tanstack-row-model', { start, end, detail: { rowCount: rows.length } }) } catch { /* optional User Timing detail */ }
  return rows
}
