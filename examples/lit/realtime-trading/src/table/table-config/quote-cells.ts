...
export const quoteRenderDiagnostics = {
  cellRendererCalls: 0, componentRenderCalls: 0,
...
abstract class QuoteElement extends LitElement {
  ...
  connectedCallback() { super.connectedCallback(); quoteCellLifecycle.created++ }
  disconnectedCallback() { quoteCellLifecycle.destroyed++; super.disconnectedCallback() }
  protected recordRender() { quoteRenderDiagnostics.componentRenderCalls++; quoteRenderDiagnostics.componentRenderCallsByName[this.componentName]++ }
}
...
export class PriceCell extends QuoteElement {
  ...
  protected render() { this.recordRender(); return html`<button class="price-button ${this.move >= 0 ? 'quote-up' : 'quote-down'}" @click=${this.select}>${this.price.toFixed(2)}</button>` }
}
...
abstract class MoveCell extends QuoteElement {
  ...
  protected indicator = ''
  protected render() { this.recordRender(); const direction = this.direction ?? (this.move >= 0 ? 'up' : 'down'); return html`<span class="move-cell quote-${direction}">${this.indicator}${signed(this.move)}</span>` }
}
@customElement('quote-stable-move') export class StableMoveCell extends MoveCell { protected readonly componentName = 'StableMoveCell' }
@customElement('quote-up-move') export class UpMoveCell extends MoveCell { protected readonly componentName = 'UpMoveCell'; protected direction = 'up' as const; protected indicator = '▲ ' }
@customElement('quote-down-move') export class DownMoveCell extends MoveCell { protected readonly componentName = 'DownMoveCell'; protected direction = 'down' as const; protected indicator = '▼ ' }
...
export class PercentChangeCell extends QuoteElement {
  ...
  protected render() { this.recordRender(); return html`<span class="percent-change-cell ${this.value >= 0 ? 'quote-up' : 'quote-down'}">${signed(this.value)}%</span>` }
}
...
export class SparklineCell extends QuoteElement {
  ...
  protected render() {
    ...
    const range = this.values.reduce((current, value) => ({ min: Math.min(current.min, value), max: Math.max(current.max, value) }), { min: first, max: first })
    const height = range.max - range.min || 1
    const denominator = Math.max(1, this.values.length - 1)
    const points = this.values.map((value, index) => `${((index / denominator) * 100).toFixed(1)},${(22 - ((value - range.min) / height) * 20).toFixed(1)}`).join(' ')
