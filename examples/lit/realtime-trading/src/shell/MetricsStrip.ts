...
export class MetricsStrip extends ControllerElement {
  ...
  protected render() {
    ...
    return html`<section class="metrics-strip" aria-label="Live performance metrics">${repeat(items, (item) => item[0], (item) => html`<article><span>${item[0]}</span><strong data-testid=${item[3] || undefined}>${item[1]}</strong><small>${item[2]}</small></article>`)}</section>`
  }
}
