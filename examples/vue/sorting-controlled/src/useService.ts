...
export default function useService(pagination: Ref<PaginationState>, sorting: Ref<SortingState>) {
  const data = ref(null)
  ...
  const requestParams = computed(() => {
    ...
    return {
      ...
      _sort: sorting.value.map((s) => s.id).join(','),
      _order: sorting.value.map((s) => s.desc ? 'desc' : 'asc').join(',')
    }
  })
