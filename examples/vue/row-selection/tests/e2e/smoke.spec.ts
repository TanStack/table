...
/** `makeData(20)` rendered in full: this example has no pagination. */
const TOTAL_ROWS = '20'
const PAGE_SIZE = 20
...
/** Bound to `getIsAllRowsSelected`. Columns are grouped, so the checkbox is
 *  not in the first header cell of the leaf row. */
...
test('selects every row on the current page from the footer', async ({
  ...
  await expectIndeterminate(footerCheckbox(page), false)
  // There is no pagination here, so the page is the whole table and the
  // select-all header agrees with the footer rather than going partial.
