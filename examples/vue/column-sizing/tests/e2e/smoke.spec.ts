...
test('renders each column at its declared size', async ({ page }) => {
  ...
  for (const [index, column] of LEAF_COLUMNS.entries()) {
    ...
    // Layout rounds, so allow a pixel of slack rather than an exact match.
    expect(Math.abs((await widthOf(page, index)) - expected)).toBeLessThan(2)
  }
...
test('resizes a column from the size panel', async ({ page }) => {
  ...
  expect(Math.abs((await widthOf(page, 0)) - 300)).toBeLessThan(2)

  // Its neighbours keep their declared widths.
  expect(Math.abs((await widthOf(page, 2)) - 100)).toBeLessThan(2)
...
test('keeps all three table renderings the same width', async ({ page }) => {
  ...
  const native = await nativeHeader(page, 4).boundingBox()
  const relative = await relativeDivHeader(page, 4).boundingBox()
  const absolute = await absoluteDivHeader(page, 4).boundingBox()

  expect(Math.abs((native?.width ?? 0) - 260)).toBeLessThan(2)
  expect(Math.abs((relative?.width ?? 0) - 260)).toBeLessThan(2)
  expect(Math.abs((absolute?.width ?? 0) - 260)).toBeLessThan(2)
...
test('keeps column sizes when data is regenerated', async ({ page }) => {
  ...
  expect(Math.abs((await widthOf(page, 2)) - 180)).toBeLessThan(2)
