...
import type { PaginationState, ColumnFiltersState } from '@tanstack/vue-table'
...
export default function useService(
  ...
  const filterParams = computed(() => {
    const params: [string, string | number | unknown][] = []

    filters.value.forEach(({ id, value }) => {
      if (typeof value === 'string') {
        params.push([`${id}_like`, value])
      } else if (
        Array.isArray(value) &&
        value.length === 2 &&
        ...
        if (typeof value[0] === 'number') {
          params.push([`${id}_gte`, value[0]])
        }
        if (typeof value[1] === 'number') {
          params.push([`${id}_lte`, value[1]])
        }
      } else if (value) {
