...
class PriceCell extends QuoteElement {
  ...
  protected renderCell() {
    super.renderCell()
    const price = this.number('price')
    const move = this.number('move')
    this.innerHTML = `<button class="price-button ${move >= 0 ? 'quote-up' : 'quote-down'}">${price.toFixed(2)}</button>`
  }
}
abstract class MoveCell extends QuoteElement {
  ...
  protected renderCell() {
    super.renderCell()
    const move = this.number('move')
    const direction = this.direction ?? (move >= 0 ? 'up' : 'down')
    this.innerHTML = `<span class="move-cell quote-${direction}">${this.indicator}${signed(move)}</span>`
  }
}
class StableMoveCell extends MoveCell {
  protected readonly componentName = 'StableMoveCell'
}
class UpMoveCell extends MoveCell {
  protected readonly componentName = 'UpMoveCell'
  protected direction = 'up' as const
  protected indicator = '▲ '
}
class DownMoveCell extends MoveCell {
...
class PercentChangeCell extends QuoteElement {
  ...
  protected renderCell() {
    ...
    this.innerHTML = `<span class="percent-change-cell ${value >= 0 ? 'quote-up' : 'quote-down'}">${signed(value)}%</span>`
  }
}
class SparklineCell extends QuoteElement {
  ...
  protected renderCell() {
    super.renderCell()
    this.innerHTML = `<svg class="sparkline ${this.getAttribute('rising') === 'true' ? 'quote-up' : 'quote-down'}" viewBox="0 0 100 24" preserveAspectRatio="none"><polyline points="${this.getAttribute('points') ?? ''}"></polyline></svg>`
  }
}

const definitions = [
  ['quote-price-cell', PriceCell],
  ['quote-stable-move', StableMoveCell],
  ['quote-up-move', UpMoveCell],
  ['quote-down-move', DownMoveCell],
  ['quote-percent-change', PercentChangeCell],
  ['quote-sparkline', SparklineCell],
] as const
