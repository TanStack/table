...
async function readSpanReport(table: Locator): Promise<SpanReport> {
  return table.evaluate((tableEl) => {
    ...
    rows.forEach((rowEl, rowIndex) => {
      // Alpine's `x-for` leaves its `<template>` elements in the row, so
      // filter to real cells instead of reading `rowEl.children` directly.
      const cells = Array.from(
        rowEl.querySelectorAll(':scope > td'),
      ) as Array<HTMLTableCellElement>
