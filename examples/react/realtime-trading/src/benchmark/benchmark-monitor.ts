...
const userTiming = {
  entryCount: 0,
  measureCallsByName: {} as Record<string, number>,
}
...
export function recordMeasure(
  ...
  const measureCall = (userTiming.measureCallsByName[name] ?? 0) + 1
  userTiming.measureCallsByName[name] = measureCall
  if ((measureCall - 1) % 20 !== 0) return
...
export class BenchmarkMonitor {
  readonly #runtime = {
    ...
    pendingCommitStartedAt: null as number | null,
  ...
  markCommitPending(): void {
    this.#runtime.pendingCommitStartedAt ??= performance.now()
  }

  recordDomCommit(): void {
    const runtime = this.#runtime
    if (runtime.pendingCommitStartedAt !== null) {
      const commitEndedAt = performance.now()
      const duration = commitEndedAt - runtime.pendingCommitStartedAt
      ...
      recordMeasure(
        ...
        runtime.pendingCommitStartedAt,
        commitEndedAt,
        {},
      )
      runtime.pendingCommitStartedAt = null
  ...
  publish(now: number): FeedMetrics {
    ...
    const metrics: FeedMetrics = {
      ...
      estimatedFrameRate: calculateFrameRate(
      ...
      profilerP95ActualMs: sortedProfilerSamples[profilerP95Index] ?? 0,
  ...
  reset(): void {
    ...
    runtime.sampleStartedAt = runtime.sessionStartedAt
    runtime.pendingCommitStartedAt = null
