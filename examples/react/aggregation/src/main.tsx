...
// demo purposes - optionally pass in custom rows to aggregate
function getAggregationRows(table: Table<typeof features, Sale>) {
  ...
  if (rowSource === 'selected') {
    return table.getFilteredSelectedRowModel().rows
  }
  if (rowSource === 'custom') {
    return table.getCoreRowModel().rows.slice(0, 3)
  }
  return undefined
}
...
function App() {
  ...
  const [rowSource, setRowSource] = React.useState<RowSource>('filtered')
    ...
    <div className="demo-root">
      ...
      <div className="controls">
        ...
        <label>
          ...
          <select
            value={rowSource}
            onChange={(event) => setRowSource(event.target.value as RowSource)}
          >
