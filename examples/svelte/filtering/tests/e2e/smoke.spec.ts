...
/** Reads `Showing <page> of <filtered> Rows`, whose second number is the
 *  pre-paginated count and so reflects filtering but not the page window. */
function rowCountLine(page: Page) {
  return page.getByText(/Showing [\s\S]* Rows/)
}

async function readFilteredRowCount(page: Page) {
  const text = (await rowCountLine(page).textContent()) ?? ''
  const numbers = text.match(/[\d,]+/g) ?? []
  return Number((numbers[numbers.length - 1] ?? '0').replace(/,/g, ''))
}
...
test('renders the table without crashing', async ({ page }) => {
  ...
  await expect(page.locator('thead tr').last().locator('th')).toHaveCount(6)
...
test('filters a numeric column by range', async ({ page }) => {
  ...
  // This example coerces the bound to a number and leaves the unset end as an
  // empty string, which keeps the range open ended.
  await expectColumnFilters(page, [{ id: 'age', value: [20, ''] }])
