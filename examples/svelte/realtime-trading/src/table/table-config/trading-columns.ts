...
export const tradingColumns: Array<TradingColumnDefinition> = [
  ...
  { id: 'priceAndChange', header: 'Price & Change', columns: [
    { id: 'price', header: 'Price', size: 96, accessorFn: (row) => row.price, sortFn: 'basic', cell: ({ row }) => recordCellRender('Last', renderComponent(LastPriceCell, { quote: row.original })) },
    { id: 'change', header: 'Chg', size: 94, accessorFn: getDayChange, cell: ({ row }) => recordCellRender('Change', renderComponent(DayChangeCell, { quote: row.original })) },
    { id: 'changePercent', header: 'Chg%', size: 90, accessorFn: getDayChangePercent, cell: ({ row }) => recordCellRender('ChangePercent', renderComponent(PercentChangeCell, { value: getDayChangePercent(row.original) })) },
  ] },
  { id: 'orderBook', header: 'Order Book', columns: [
    ...
    { id: 'bidSize', header: 'Bid Vol', size: 100, accessorFn: (row) => row.bidSize, cell: ({ row }) => recordCellRender('BidVolume', compactFormatter.format(row.original.bidSize)) },
    { id: 'ask', header: 'Ask', size: 90, accessorFn: (row) => row.ask, cell: ({ row }) => recordCellRender('Ask', row.original.ask.toFixed(2)) },
    { id: 'askSize', header: 'Ask Vol', size: 100, accessorFn: (row) => row.askSize, cell: ({ row }) => recordCellRender('AskVolume', compactFormatter.format(row.original.askSize)) },
  ] },
  ...
  { id: 'chart', header: 'Chart', columns: [
    { id: 'history', header: 'Intraday', size: 150, enableSorting: false, cell: ({ row }) => recordCellRender('Intraday', renderComponent(SparklineCell, { values: row.original.history })) },
  ] },
]
...
export function readMeasuredRows<Row>(readRows: () => Array<Row>): Array<Row> {
  ...
  rowModelDiagnostics.maxDurationMs = Math.max(rowModelDiagnostics.maxDurationMs, duration)
  rowModelDiagnostics.lastRowCount = rows.length
  try { performance.measure('tanstack-row-model', { start, end, detail: { rowCount: rows.length } }) } catch { /* User Timing Level 3 is optional. */ }
  return rows
}
