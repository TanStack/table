...
<aside id="benchmark-configurator" class="configurator" aria-label="Benchmark configurator">
  ...
  <section class="config-section" aria-labelledby="feed-settings"><h2 id="feed-settings">FEED</h2>
    ...
    <label class="field rate-field"><span>Synthetic quote workload <strong data-testid="target-sample-rate">{rate.format(targetTicksPerSecond.current)} samples/s</strong></span><input data-testid="target-rate-slider" type="range" min="0" max={feedSampleRateOptions.length - 1} step="1" value={feedSampleRateIndex(targetTicksPerSecond.current)} onchange={(event) => feed.actions.setTargetRate(feedSampleRateAt(numberValue(event)))} /><small>Fixed worker-side workload levels. Samples are coalesced; this is not the message delivery rate.</small></label>
