...
// This example uses the new `createTableHook` method to create a reusable table
// factory instead of the standalone `useTable` hook and `createColumnHelper`.
...
// 3. Tell the hook which features to share across every table it creates. This
// is a basic table with no additional features.
const features = tableFeatures({})

const { createAppColumnHelper, createAppTable } = createTableHook({
  features,
})

// 4. Create a column helper (pre-bound to the hook's features).
...
export default class BasicAppTable extends Component {
  ...
  // No `features` here: they come from the hook.
