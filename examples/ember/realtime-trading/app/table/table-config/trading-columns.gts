...
export function createTradingColumns(
  ...
  return [
    ...
    { id: 'priceAndChange', header: 'Price & Change', columns: [
      { id: 'price', header: 'Price', size: 96, accessorFn: (row) => row.price, sortFn: 'basic', cell: () => recordCellRender('Last', flexRenderComponent(PriceCell, { selectSymbol: controller.actions.selectSymbol })) },
      { id: 'change', header: 'Chg', size: 94, accessorFn: getDayChange, cell: ({ row }) => recordCellRender('Change', rendererMode === 'stable' ? flexRenderComponent(StableMoveCell) : getDayChange(row.original) >= 0 ? flexRenderComponent(UpMoveCell) : flexRenderComponent(DownMoveCell)) },
      { id: 'changePercent', header: 'Chg%', size: 90, accessorFn: getDayChangePercent, cell: () => recordCellRender('ChangePercent', flexRenderComponent(PercentChangeCell)) },
    ] },
    ...
    { id: 'chart', header: 'Chart', columns: [
      { id: 'history', header: 'Intraday', size: 150, enableSorting: false, cell: () => recordCellRender('Intraday', flexRenderComponent(SparklineCell)) },
    ] },
  ]
}
...
export function getDayChangePercent(quote: MarketQuote): number {
  return quote.previousClose === 0 ? 0 : (getDayChange(quote) / quote.previousClose) * 100
}
