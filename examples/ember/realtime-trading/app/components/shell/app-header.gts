...
export default class AppHeader extends Component<Signature> {
  ...
  get status() {
    return !this.feedState.workerReady ? 'FEED CONNECTING' : this.feedState.running ? 'FEED LIVE' : 'FEED PAUSED'
  }
  <template>
    <header class='app-bar'>
      <div class='brand'><strong>MARKET MONITOR</strong></div>
      <div class='header-actions'>
        <span class='feed-status {{if (and this.feedState.workerReady this.feedState.running) "is-running"}}' data-testid='feed-status'>
          <span class='status-dot' aria-hidden='true'></span>{{this.status}}
        </span>
        <button class='sidebar-toggle' type='button' aria-expanded={{@sidebarOpen}} aria-controls='benchmark-configurator' aria-label={{if @sidebarOpen 'Close configurator' 'Open configurator'}} {{on 'click' @toggleSidebar}}>
