...
export default class TradingShell extends Component<Signature> {
  ...
  get rootClass() { return `trading-terminal${this.sidebarOpen ? '' : ' is-sidebar-collapsed'}` }
  <template>
    <main class={{this.rootClass}}>
      <div class='shell-header'>
        <AppHeader @feed={{@feed}} @sidebarOpen={{this.sidebarOpen}} @toggleSidebar={{this.toggleSidebar}} />
        {{#if isDevelopment}}<aside class='development-warning'>DEV BUILD — use the production build before recording results.</aside>{{/if}}
      </div>
      ...
      <div class='sidebar-slot'>{{#if this.sidebarOpen}}<Configurator @controller={{@controller}} @feed={{@feed}} />{{/if}}</div>
    </main>
  </template>
}
