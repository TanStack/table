...
export default class SelectedInstrument extends Component<Signature> {
  ...
  constructor(owner: Owner, args: Signature['Args']) {
    ...
    observeValue(this, args.controller.renderAtoms.selectedSymbol, (symbol) => { this.symbol = symbol })
  }
  get quote(): MarketQuote | null { return this.args.feed.getQuoteBySymbol(this.feedState.quotes, this.symbol) }
  <template>
    <section class='config-section selected-instrument' data-testid='selected-instrument'>
      <h2>SELECTED INSTRUMENT</h2>
      {{#if this.quote}}
        <div class='selection'><div><strong>{{this.quote.symbol}}</strong><span>{{this.quote.company}}</span></div><small>{{this.quote.venue}}</small></div>
        <dl><div><dt>Last</dt><dd>{{fixed this.quote.price}}</dd></div><div><dt>Bid / ask</dt><dd>{{fixed this.quote.bid}} / {{fixed this.quote.ask}}</dd></div></dl>
      {{else}}
