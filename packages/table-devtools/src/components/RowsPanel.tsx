...
import type { Cell, Column, Row, RowData, TableFeatures } from '@tanstack/table-core'
...
export function RowsPanel() {
  ...
  const [selectedRowModel, setSelectedRowModel] = createSignal<
    (typeof ROW_MODEL_GETTERS)[number]
  >('getRowModel')
  ...
  const getAllRows = (): Array<AnyRow> => {
    tableState?.()
    selectedRowModel()
    const getter = tableInstance?.[
      selectedRowModel()
    ] as (() => { rows: Array<AnyRow> }) | undefined
    return getter?.().rows ?? []
  }
          ...
          <>
            <div class={styles().sectionTitle}>
              ...
              {getRawDataTotalCount() > ROW_LIMIT && (
                <span class={styles().rowLimitNote}> (First {ROW_LIMIT} rows)</span>
              )}
            </div>
