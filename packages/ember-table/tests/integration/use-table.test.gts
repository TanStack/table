  ...
  test('row model reacts to tracked data changes', async (assert) => {
    ...
    class TableComponent extends Component {
      ...
      <template>
        ...
        {{/each}}
        <button type="button" data-test-shrink {{on "click" this.shrink}}>shrink</button>
      </template>
    }

    await render(<template><TableComponent /></template>)

    assert.dom('[data-test-row]').exists({ count: 3 }, 'renders a row per data item');

    await click('[data-test-shrink]');

    assert.dom('[data-test-row]').exists(
      { count: 1 },
      'row model re-renders when tracked data changes'
    );
  })
  ...
  test('exposes expected table structure', async (assert) => {
    class TableComponent extends Component {
      ...
      <template>
        ...
        <div data-test-keys-getrowmodel>{{keysInclude this.table "getRowModel"}}</div>
    ...
    assert.dom('[data-test-has-options]').hasText('true', 'table has an "options" property');
    assert.dom('[data-test-has-getrowmodel]').hasText('true', 'table has a "getRowModel" method');
    assert.dom('[data-test-keys-getrowmodel]').hasText('true', 'Object.keys includes getRowModel');
    assert.dom('[data-test-keys-options]').hasText('true', 'Object.keys includes options');

    assert.dom('[data-test-has-notfound]').hasText('false', 'unknown properties are absent');
  })
  ...
  test('paginated row model reacts to pagination state', async (assert) => {
    ...
    class TableComponent extends Component {
      ...
      <template>
        ...
        <button type="button" data-test-shrink-page {{on "click" this.shrinkPage}}>shrink</button>
      </template>
    }

    await render(<template><TableComponent /></template>)

    assert.dom('[data-test-row]').exists({ count: 5 }, 'renders one page of rows');
    assert.dom('[data-test-core-count]').hasText('10', 'core row model holds every row');
    ...
    assert.dom('[data-test-core-count]').hasText('10', 'core row model is unaffected by pagination');
  })
});
