---
title: Cell Spanning (Angular) Guide
---
...
`cell.getIsCovered()` is a convenience for the same check, so `@if (!cell.getIsCovered())` also works when you do not need the span numbers separately.
