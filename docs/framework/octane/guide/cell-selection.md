---
title: Cell Selection (Octane) Guide
---

## Examples

The cell-selection APIs work with Octane, but there is not currently a dedicated
Octane example. The existing React and Preact examples depend on an additional
TanStack framework package, so they are intentionally outside the Octane example
porting scope.
...
```ts
import { useHotkeys } from '@tanstack/octane-hotkeys'
...
Octane re-renders a component subtree when its state changes, so a drag that
