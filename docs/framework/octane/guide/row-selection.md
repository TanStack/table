---
title: Row Selection (Octane) Guide
---
...
Bind an Octane checkbox handler with `onClick`, not `onChange`, so the handler receives the click event and its `shiftKey` modifier.
