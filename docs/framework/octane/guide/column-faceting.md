...
```ts
// Refreshed whenever the facet request resolves; read live below
const facets = { data: await fetchFacets(activeFilters) }
...
const features = tableFeatures({
  ...
  facetedUniqueValues: (table, columnId) => () => {
    const facetingData = table.options.meta?.facetingData
    return new Map<string, number>(facetingData?.uniqueValues[columnId] ?? [])
  },
  facetedMinMaxValues: (table, columnId) => () => {
    return table.options.meta?.facetingData?.minMaxValues[columnId]
  },
})
