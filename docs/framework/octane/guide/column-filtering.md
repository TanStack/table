---
title: Column Filtering (Octane) Guide
---
...
If you are using the built-in client-side filtering features, add the `columnFilteringFeature`, the `filteredRowModel` factory, and a `filterFns` registry to your `tableFeatures` call. Import `createFilteredRowModel` and the filter functions you need from TanStack Table:
