---
title: Cell Spanning (Preact) Guide
---
...
`cell.getIsCovered()` is a convenience for the same check, so `cell.getIsCovered() ? null : ...` also works when you do not need the span numbers separately.
