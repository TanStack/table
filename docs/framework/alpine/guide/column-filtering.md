---
title: Column Filtering (Alpine) Guide
---
...
The table's state atoms are reactive in Alpine. `table.atoms.columnFilters.get()` is a reactive read when used inside an Alpine binding (`x-text`, `x-html`, `:value`, `x-if`, `x-for`, `x-effect`, or a getter/method on your `Alpine.data` object); in event handlers and other untracked code, the same call simply returns the current value. `table.store.get()` returns a current full-state snapshot, useful for debugging.
...
table.atoms.columnFilters.get() // reactive read inside Alpine bindings, plain read elsewhere
```
...
const table = createTable({
  ...
  state: {
    get columnFilters() {
      return local.columnFilters // connect the reactive slice back down to the table
    },
  },
  onColumnFiltersChange: (updater) => {
    local.columnFilters =
      typeof updater === 'function' ? updater(local.columnFilters) : updater
  },
})
```
...
<th>
  <template x-if="!header.isPlaceholder">
    <div>
      <div x-html="FlexRender({ header })"></div>
      <template x-if="header.column.getCanFilter()">
        <input
          type="text"
          placeholder="Search..."
...
For a numeric range filter, store a `[min, max]` tuple and update each bound separately. Reading and writing each bound is small enough to keep inline, or you can expose helpers on your `Alpine.data` object.
...
Alpine.data('table', () => {
  ...
  return {
    table,
    FlexRender,
    rangeValue(column, index) {
      return column.getFilterValue()?.[index] ?? ''
    },
    setRangeMin(column, value) {
      column.setFilterValue((old) => [
        value === '' ? undefined : Number(value),
        old?.[1],
      ])
    },
    setRangeMax(column, value) {
      column.setFilterValue((old) => [
        old?.[0],
        value === '' ? undefined : Number(value),
      ])
...
<div class="filter-row">
  <input
    type="number"
    placeholder="Min"
    :value="rangeValue(header.column, 0)"
    @input="setRangeMin(header.column, $event.target.value)"
  />
  <input
    type="number"
    placeholder="Max"
    :value="rangeValue(header.column, 1)"
    @input="setRangeMax(header.column, $event.target.value)"
  />
</div>
```
...
- **`column.setFilterValue`** - Useful for connecting filter inputs to their `input` or `change` handlers.
