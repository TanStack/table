...
```html
<select>
  <template x-for="[value, count] in suggestions" :key="String(value)">
    <option
      :value="String(value)"
      x-text="`${String(value)} (${count})`"
    ></option>
  </template>
</select>
```
...
### Reactive Facet Controls in Alpine

Expose facet reads as methods on the object returned by `Alpine.data`, then call those methods from Alpine bindings. The bindings reevaluate when the adapter's reactive table state changes.
...
Alpine.data('table', () => {
  ...
  return {
    ...
    facetSelected(column, value) {
      return (column.getFilterValue() ?? []).includes(value)
    },
    toggleFacet(column, value) {
...
```html
<template
  x-for="[value, count] in facetValues(header.column)"
  :key="String(value)"
>
  <label>
    <input
      type="checkbox"
      :checked="facetSelected(header.column, value)"
      @change="toggleFacet(header.column, value)"
    />
    <span x-text="String(value)"></span>
...
```ts
facetRange(column) {
  return column.getFacetedMinMaxValues() ?? [0, 1]
}
```
...
<input
  type="range"
  :min="facetRange(header.column)[0]"
  :max="facetRange(header.column)[1]"
  :value="header.column.getFilterValue() ?? 0"
  @input="header.column.setFilterValue(Number($event.target.value))"
/>
```
...
const storageColumn: ColumnDef<typeof features, Account> = {
