...
A covered cell reports a span of `0`, and the renderer skips it by returning `nothing`. This is the same convention as [`header.rowSpan`](../../../guide/headers#header-row-spanning).

```ts
import { html, nothing } from 'lit'
import { repeat } from 'lit/directives/repeat.js'

// inside render(), for each row:
html`
  <tr>
    ${repeat(
      row.getVisibleCells(),
      ...
      (cell) => {
        ...
        // left. Skip it. Do not render rowspan="0": in HTML that means
        // "span to the end of the row group", which merges the cell down
        // the entire tbody.
        if (rowSpan === 0 || colSpan === 0) return nothing
...
`cell.getIsCovered()` is a convenience for the same check, so `cell.getIsCovered() ? nothing : ...` also works when you do not need the span numbers separately.
