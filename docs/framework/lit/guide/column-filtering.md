...
If you are using the built-in client-side filtering features, add the `columnFilteringFeature` to your features and the `filteredRowModel` factory as a slot on `tableFeatures`. Import `createFilteredRowModel` and the filter functions you need from TanStack Table:
