...
return html`
  <select>
    ${suggestions.map(
      ([value, count]) => html`
        <option value=${String(value)}>${String(value)} (${count})</option>
      `,
    )}
  </select>
`
...
### Reactive Facet Controls in Lit

A `TableController` subscribes its host to table state changes. For a smaller reactive template region, use `table.subscribe` and select the filter state that affects the facet values.

```ts
renderFacetOptions(table, column) {
