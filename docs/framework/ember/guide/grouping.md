...
const features = tableFeatures({
  columnGroupingFeature,
  groupedRowModel: createGroupedRowModel(),
  aggregationFns: { ...aggregationFns, myCustomAggregation },
})
...
```ts
import { useTable, createAtom, type GroupingState } from '@tanstack/ember-table'

const groupingAtom = createAtom<GroupingState>([])

// read the atom value wherever you need it (such as a query key)
