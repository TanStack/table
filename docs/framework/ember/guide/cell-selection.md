---
title: Cell Selection (Ember) Guide
---
...
- `table.atoms.cellSelection.get()` - returns the current cell selection (a tracked read, so it invalidates templates and getters automatically)
...
Reads of `table.atoms.cellSelection.get()` are tracked inside Ember reactive contexts, so they stay fresh automatically. Outside one, the same call is a plain snapshot.
...
```ts
@tracked cellSelection: CellSelectionState = []
...
table = useTable(() => ({
  ...
  onCellSelectionChange: (updater) => {
    this.cellSelection =
      typeof updater === 'function' ? updater(this.cellSelection) : updater
  },
}))
```
...
```gts
{{! Ember templates extract function references without binding, so each
    handler goes through a helper that calls it on the right cell }}
<td
...
// Ember has no hotkeys adapter, so the framework-agnostic core handler is used
onGridKeyDown = createMultiHotkeyHandler({
  ArrowUp: () => this.table.moveCellSelection('up'),
  ArrowDown: () => this.table.moveCellSelection('down'),
  'Shift+ArrowDown': () => this.table.extendCellSelection('down'),
  'Mod+A': () => this.table.selectAllCells(),
  Escape: () => this.table.resetCellSelection(true),
})
...
Ember's tracked signals invalidate only the getters and template sections that
actually read the selection, so the example renders its cells plainly. There is
...
roughly 19ms per move with plain reads.
...
table does become a bottleneck, paginate or virtualize so fewer rows exist in the
DOM, rather than reaching for a subscription pattern.
