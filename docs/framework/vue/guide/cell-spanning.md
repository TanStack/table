...
`cell.getIsCovered()` is a convenience for the same check, so `v-if="!cell.getIsCovered()"` also works when you do not need the span numbers separately.
