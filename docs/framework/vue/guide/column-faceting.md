...
<template>
  <select>
    <option
      v-for="[value, count] in suggestions"
      :key="String(value)"
      :value="String(value)"
    >
      {{ String(value) }} ({{ count }})
...
<template>
  <label v-for="[value, count] in values" :key="String(value)">
    <input
      type="checkbox"
...
const range = computed(() => props.column.getFacetedMinMaxValues() ?? [0, 1])
const currentValue = computed(() => props.column.getFilterValue() ?? 0)
</script>
